@foreach ($models as $model)
    <tr>
        @if ($padding)
            <td style="padding-left: {{ $padding }}px"><span data-feather="corner-down-right"></span> {{ $model->name }}</td>
        @else
            <td>{{ $model->name }}</td>
        @endif
        <td class="text-right">
    ...
    @if ($model->childrens)
        @includeIf('admin.template.table_row', ['models' => $model->childrens, 'padding' => $padding + 18])
    @endif
@endforeach
